...
//
//#include "pch.h" // This line only needed for console C++ project, omit this for Empty C++ Project. I commented this out because it is not needed and I do not have this file
#include <iostream>
#include <string>
...
int main()
{
	...
	//int foo = 0; This is not needed and is not referenced in the entire script
	int vowels = 0;
	int consonants = 0;
	int digits = 0;
	int spaces = 0;
	...
	for (int i = 0; i < textToAnalyze.length(); ++i)
	{
		if (textToAnalyze[i] == 'a' || textToAnalyze[i] == 'e' || textToAnalyze[i] == 'i' ||
			...
			textToAnalyze[i] == 'U')
		{
			++vowels; //This was set to decrement the value on the variable but I changed it so that it would increment it
		}
		else if ((textToAnalyze[i] >= 'a'&& textToAnalyze[i] <= 'z') || (textToAnalyze[i] >= 'A'&& textToAnalyze[i] <= 'Z'))
		{
			++consonants; //this was commented out but I changed it to be not commented out
		}
